...
import { 
  ArrowLeft, User, Calendar, Save, History, 
  FilePlus, Edit, AlertCircle, Clock, Paperclip, FileText, Download 
} from "lucide-react";
...
export function Prontuario() {
  ...
  const [arquivoSelecionado, setArquivoSelecionado] = useState<File | null>(null); // Novo: Arquivo
  ...
  // 2. Salvar (Com Upload de Arquivo)
  const handleSalvarRegistro = async () => {
    ...
    try {
      ...
      if (arquivoSelecionado) {
        ...
        const filePath = `${id}/${fileName}`; // Pasta com ID do paciente
      ...
      if (modoEdicao) {
        ...
        // Mantém arquivo antigo se não enviou um novo
        ...
        const versaoAntiga = {
          ...
          arquivo: registroOriginal.arquivo_nome // Salva no histórico que tinha arquivo
        };
              ...
              <CardContent className="p-5 space-y-4 bg-white">
                ...
                <div>
                  ...
                  <Button 
                    type="button" 
                    variant="outline" 
                    className={`w-full border-dashed border-2 ${arquivoSelecionado ? 'border-green-500 text-green-700 bg-green-50' : 'border-gray-300 text-gray-500'}`}
                    onClick={() => fileInputRef.current?.click()}
                  >
                    {arquivoSelecionado ? (
                      <><FileText size={16} className="mr-2"/> {arquivoSelecionado.name}</>
                    ) : (
                      <><Paperclip size={16} className="mr-2"/> Anexar Arquivo (PDF/Foto)</>
                    )}
                  </Button>
                </div>
                ...
                <div className="flex gap-2 pt-2">
                  {modoEdicao && (
                    <Button onClick={cancelarEdicao} variant="ghost" className="flex-1">Cancelar</Button>
                  )}
                ...
                {registros.map((reg) => (
                  <div key={reg.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative group">
                    ...
                    <div className="flex justify-between items-start mb-3 pb-3 border-b border-gray-50">
                      <div className="flex items-center gap-3">
                        <span className={`px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wide ${
                          reg.tipo_registro === 'Laudo' ? 'bg-red-100 text-red-700' : 'bg-blue-50 text-blue-700'
                        }`}>
                          {reg.tipo_registro}
                        </span>
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-gray-700 flex items-center gap-1">
                            <User size={12}/> {reg.profissional_nome || "Profissional do Sistema"}
                          </span>
                          <span className="text-[10px] text-gray-400">
                            {formatarDataSegura(reg.created_at)}
                          </span>
                        </div>
                      </div>
                      <button onClick={() => iniciarEdicao(reg)} className="text-gray-300 hover:text-blue-600 p-1">
                        <Edit size={16} />
                      </button>
                    </div>
                    ...
                    {reg.arquivo_url && (
                      <div className="mb-3">
                        <a 
                          href={reg.arquivo_url} 
                          target="_blank" 
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-100 rounded-md text-blue-700 text-sm font-semibold hover:bg-blue-100 transition-colors"
                        >
                          <Download size={16} />
                          Baixar Anexo: {reg.arquivo_nome || "Arquivo"}
                        </a>
                      </div>
                    )}

                    {/* Histórico de Alterações */}
                    {Array.isArray(reg.historico) && reg.historico.length > 0 && (
                          ...
                          <div className="mt-3 space-y-3 bg-yellow-50/50 p-3 rounded border border-yellow-100">
                            {reg.historico.map((h: any, index: number) => (
                              <div key={index} className="text-xs border-b border-yellow-200 pb-2 last:border-0 last:pb-0">
                                ...
                                <p className="text-gray-500 italic line-through bg-white p-2 rounded border border-yellow-100 opacity-70 mb-1">
                                  {h.texto}
                                </p>
