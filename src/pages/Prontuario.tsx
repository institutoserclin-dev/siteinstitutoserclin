...
import { 
  ...
  FileEdit, ClipboardList, History // Adicionado History
} from "lucide-react";
...
export function Prontuario() {
  ...
  // --- NOVA FUNÇÃO: REGISTRAR LOG NA TABELA logs_prontuario ---
  ...
  const carregarDados = async () => {
    try {
      ...
      carregarLogs(); // Carrega os logs junto com os dados
  ...
  const handleSalvarDadosPaciente = async () => {
    ...
    try {
      ...
      await registrarLog("Editou Dados Clínicos", "Atualizou anamnese ou observações gerais.");
  ...
  const handleSalvarRegistro = async () => {
    ...
    try {
      ...
      if (modoEdicao) {
        ...
        await registrarLog("Editou Registro", `Alterou ${novoRegistro.tipo} (ID: ${modoEdicao})`);
        ...
        await registrarLog("Criou Registro", `Adicionou nova ${novoRegistro.tipo}`);
        toast.success("Salvo!");
      }
  ...
  const handleSalvarAgendamento = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await registrarLog("Novo Agendamento", `Consulta com ${formAgendamento.profissional} para ${format(dInicio, 'dd/MM HH:mm')}`);
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <Card className="border-none shadow-sm rounded-[2rem] overflow-hidden bg-white">
              ...
              <CardContent className="p-6 space-y-4 max-h-[300px] overflow-y-auto">
                {loadingLogs ? <p className="text-[10px] text-center font-bold text-gray-300">CARREGANDO...</p> : 
                 logs.length === 0 ? <p className="text-[10px] text-center font-bold text-gray-300">NENHUMA ATIVIDADE.</p> :
                 logs.map(log => (
                   <div key={log.id} className="border-l-2 border-blue-100 pl-3 py-1 hover:border-blue-500 transition-colors">
            ...
            <Card className={`border-none shadow-lg rounded-[2rem] overflow-hidden ${modoEdicao ? 'ring-4 ring-amber-400' : ''}`}>
              ...
              <CardContent className="p-6 space-y-4">
                <select className="w-full rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 text-xs font-bold uppercase outline-none" value={novoRegistro.tipo} onChange={e => setNovoRegistro({...novoRegistro, tipo: e.target.value})}><option value="Sessão">Sessão</option><option value="Laudo">Laudo / PDF</option><option value="Avaliação">Avaliação</option></select>
          ...
          <div className="lg:col-span-2 space-y-4">
            {registros.length === 0 ? <div className="bg-white p-12 rounded-[2rem] border border-dashed border-gray-200 text-center"><p className="text-gray-400 font-bold uppercase text-xs">Nenhum registro encontrado.</p></div> :
            registros.map((reg) => (
              <div key={reg.id} className="bg-white p-6 pl-8 rounded-[1.5rem] shadow-sm border border-gray-100 relative overflow-hidden group">
                ...
                <div className="flex justify-between items-center border-b pb-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={async () => { if(confirm("Apagar registro?")) { await registrarLog("Excluiu Registro", `Removeu ${reg.tipo_registro}`); supabase.from("prontuarios").delete().eq("id", reg.id).then(carregarDados) } }} className="text-gray-200 hover:text-red-400"><Trash2 size={14}/></button>
                    <button onClick={() => iniciarEdicao(reg)} className="text-gray-300 hover:text-amber-500"><Edit size={16}/></button>
                  </div>
                </div>
                <p className="text-sm text-gray-600 whitespace-pre-wrap mt-2 leading-relaxed">{reg.descricao}</p>
                {reg.arquivo_url && <a href={reg.arquivo_url} target="_blank" className="inline-flex items-center gap-2 text-[10px] font-black text-[#1e3a8a] uppercase mt-3 bg-blue-50 px-4 py-2 rounded-full hover:bg-blue-100 transition-colors"><FileText size={14} /> Ver Documento Anexo</a>}
          ...
          <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-4 backdrop-blur-sm">
            <Card className="w-full max-w-[500px] rounded-[2.5rem] bg-white p-8 shadow-2xl animate-in zoom-in duration-200">
              ...
              <div className="space-y-4 text-left">
                <textarea value={tempDados.anamnese} onChange={e => setTempDados({...tempDados, anamnese: e.target.value})} className="w-full bg-gray-50 border-none rounded-xl p-4 text-sm h-32 outline-none" placeholder="Anamnese..." />
                <textarea value={tempDados.observacoes} onChange={e => setTempDados({...tempDados, observacoes: e.target.value})} className="w-full bg-gray-50 border-none rounded-xl p-4 text-sm h-24 outline-none" placeholder="Observações..." />
                <Button onClick={handleSalvarDadosPaciente} disabled={loading} className="w-full bg-[#1e3a8a] text-white font-black h-14 rounded-2xl shadow-xl uppercase text-xs">Salvar Dados Clínicos</Button>
              </div>
            </Card>
          </div>
        )}
        ...
        {isAgendamentoOpen && (
          <div className="fixed inset-0 bg-black/60 z-[999] flex items-center justify-center p-4 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && setIsAgendamentoOpen(false)}>
            <Card className="w-full max-w-[420px] rounded-[2.5rem] bg-white overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
              <div className="bg-[#1e3a8a] p-5 flex justify-between items-center rounded-t-[2.5rem]"><h3 className="font-black uppercase text-[11px] text-white">Agendar: {paciente?.nome}</h3><button onClick={() => setIsAgendamentoOpen(false)} className="text-white"><X size={22}/></button></div>
              <form onSubmit={handleSalvarAgendamento} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Select value={formAgendamento.duracao} onValueChange={(v) => setFormAgendamento({...formAgendamento, duracao: v})}><SelectTrigger className="bg-gray-50 border-none h-10 font-bold text-sm"><SelectValue /></SelectTrigger><SelectContent className="z-[1000]"><SelectItem value="30">30 Min</SelectItem><SelectItem value="40">40 Min</SelectItem><SelectItem value="60">60 Min</SelectItem></SelectContent></Select>
                </div>
                <Select value={formAgendamento.profissional} onValueChange={(v) => setFormAgendamento({...formAgendamento, profissional: v})} required><SelectTrigger className="bg-gray-50 h-10 font-bold text-sm uppercase"><SelectValue placeholder="Profissional" /></SelectTrigger><SelectContent className="z-[1000]">{equipeClinica.map(p => <SelectItem key={p.id} value={p.nome}>{p.nome}</SelectItem>)}</SelectContent></Select>
                ...
                <Button type="submit" disabled={loadingAgendamento} className="w-full bg-[#1e3a8a] text-white font-black h-14 rounded-2xl uppercase text-xs shadow-xl transition-all">Confirmar Agendamento</Button>
              </form>
            </Card>
          </div>
        )}
