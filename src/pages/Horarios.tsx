...
import { ArrowLeft, Clock, Save, User, UserCheck } from "lucide-react";
...
export function Horarios() {
  ...
  const carregarEquipe = async () => {
    ...
    if (!error && data) {
      ...
      const profissionais = data.filter(p => p.role !== 'secretaria' || p.nome.includes('Renata'));
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 pt-[calc(env(safe-area-inset-top,0px)+24px)] md:pt-6">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/sistema")} className="p-2 -ml-2 text-gray-400 hover:text-green-600 transition-colors">
              <ArrowLeft size={24} />
            </button>
            <div>
          ...
          <Button onClick={salvarHorarios} disabled={loading} className="w-full md:w-auto bg-green-600 hover:bg-green-700 text-white font-black uppercase tracking-widest text-xs h-12 px-8 rounded-2xl shadow-lg transition-all">
