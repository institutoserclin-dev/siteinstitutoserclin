  ...
  const fetchData = async () => {
    try {
      ...
      if (user && todosPerfis) {
        ...
        // BUSCA POR E-MAIL PARA GARANTIR SINCRONIA
        ...
        if (meuPerfil) {
          ...
          // LÓGICA DE MUDANÇA DE PERFIL: Se você rebaixar no banco, ehGestorEfetivo vira false aqui na hora.
          if (
        ...
        <div className="flex gap-1.5 items-center">
          {isGestorSeguro && (
            <>
              ...
              <Button onClick={() => setIsConfirmacaoAmanhaOpen(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[10px] uppercase gap-2 rounded-full px-5 h-9 mr-2 shadow-lg relative">
                <Send size={14} /> 
                Confirmar Amanhã
          ...
          {/* BOTÕES DINÂMICOS: Se você mudar o perfil no banco, eles aparecem ou somem aqui no próximo F5 */}
          {isGestorSeguro && (
            <>
          ...
          {(userEmail === 'romulochaves77@gmail.com' || userEmail === 'nahpsicologiachaves@gmail.com') && (
          ...
          }} className="bg-blue-600 hover:bg-blue-700 text-white rounded-full h-9 px-4 text-xs font-black shadow-lg">
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-[650px] border border-gray-100 overflow-hidden">
            ...
            <div className="p-4 max-h-[60vh] overflow-y-auto bg-gray-50/50 space-y-3">
              {agendamentosAmanha.length === 0 ? (
                <div className="text-center py-20"><p className="text-gray-400 font-bold uppercase text-xs">Nenhum agendamento para amanhã.</p></div>
              ) : (
                agendamentosAmanha.map((ag, idx) => (
                  <div key={idx} className="flex items-center justify-between p-5 bg-white rounded-3xl border border-gray-100 shadow-sm group">
                    <div className="flex items-center gap-5">
                      ...
                      <div className="flex flex-col text-left"><span className="font-black text-[15px] uppercase text-gray-800 leading-tight">{ag.paciente_nome}</span><div className="flex items-center gap-2 mt-1"><span className="text-[10px] font-bold text-gray-400 uppercase">Prof: {ag.profissional_nome}</span><span className="text-[10px] font-bold text-blue-500 uppercase">Sala {ag.sala_id}</span></div></div>
                    </div>
            ...
            <form onSubmit={handleSalvarAgendamento} className="p-6 space-y-4 text-left overflow-y-auto flex-1 custom-scrollbar">
              ...
              <div className="space-y-1">
                ...
                <Select value={form.profissional} onValueChange={(v) => setForm({...form, profissional: v})} required disabled={!isGestorSeguro}>
                  ...
                  <SelectContent className="z-[110]">{isGestorSeguro ? equipe.map(p => <SelectItem key={p.id} value={p.nome}>{p.nome}</SelectItem>) : <SelectItem value={nomeLogado}>{nomeLogado}</SelectItem>}</SelectContent>
                </Select>
              </div>
              <div className="space-y-1">
              ...
              <div className="pt-4 flex flex-col gap-2 shrink-0">
                {form.telefone && (<Button type="button" onClick={() => enviarWhatsApp(form.paciente_nome, form.telefone, form.profissional, form.inicio)} className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-black h-11 rounded-xl flex items-center justify-center gap-2 uppercase text-[10px] shadow-md"><MessageCircle size={16} /> Confirmar WhatsApp</Button>)}
                {eventoSelecionadoId && (<Button type="button" onClick={gerarComprovante} className="w-full bg-[#1e3a8a] hover:bg-black text-white font-black h-11 rounded-xl flex items-center justify-center gap-2 uppercase text-[10px] shadow-md"><FileText size={16} /> Gerar Atestado</Button>)}
                <div className="flex gap-2">
                  {eventoSelecionadoId && (<Button type="button" variant="outline" onClick={handleExcluirAgendamento} className="px-5 border-red-200 text-red-500 hover:bg-red-50 h-12 rounded-2xl"><Trash2 size={20} /></Button>)}
